...
export function getUserPoint() {
  return async (dispatch) => {
    dispatch(fetchData());
    try {
      const result = await axios.get(`https://taleify-literation.vercel.app/user_point`);
...
export function getUserPointByUserId(id) {
  return async (dispatch) => {
    dispatch(fetchData());
    try {
      const result = await axios.get(`https://taleify-literation.vercel.app/user_point/${id}`);
...
export function addUserPoint(id, point) {
  return async(dispatch) => {
    dispatch(fetchData());
    try {
      const result = await axios.put(`https://taleify-literation.vercel.app/user_point/${id}`,{
        point: point
      })
