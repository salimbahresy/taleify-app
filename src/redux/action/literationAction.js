...
export function fetchFilteredData(query){
  return async (dispatch)=>{
    dispatch(fetchData());
    try {
      const result = await axios.get(`https://taleify-literation-production.up.railway.app/literation/search/?title=${query}`);
...
export function getLiteration() {
  return async (dispatch) => {
    dispatch(fetchData());
    try {
      const result = await axios.get(`https://taleify-literation-production.up.railway.app/literation`);
...
export function getLiterationById(id) {
  return async (dispatch) => {
    dispatch(fetchData());
    try {
      const result = await axios.get(`https://taleify-literation-production.up.railway.app/literation/${id}`);
