...
export function getRating() {
  return async (dispatch) => {
    dispatch(fetchData());
    try {
      const result = await axios.get(`https://taleify-literation.vercel.app/rating`);
  ...
  return async (dispatch) => {
    ...
    try {
      const result = await axios.get(
        `https://taleify-literation.vercel.app/rating/literation/${literationId}`
      );
...
export function getRatingByUserLiteration(userId, literationId) {
  return async (dispatch) => {
    ...
    try {
      const result = await axios.get(
        `https://taleify-literation.vercel.app/rating/user_literation/${userId}/${literationId}`
      );
...
export function addRating(rating, literation, user) {
  return async (dispatch) => {
    dispatch(fetchData());
    try {
      const result = await axios.post(`https://taleify-literation.vercel.app/rating`, {
        rating,
        literation,
        user,
      });
